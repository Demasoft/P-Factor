...
@export_file( "*.tscn" ) var linked_scene: String : set = _onscene_set
...
@export var entrances_down : Array[ float ] = []
@export var entrances_left : Array[ float ] = []

@onready var transition_block: Control = $TransitionBlock
...
func _onscene_set( value : String ) -> void:
...
func create_entrance_data( transitions : Array[ LevelTransition ] ) -> void:
	...
	for t in transitions:
		...
		elif t.location == LevelTransition.SIDE.DOWN:
			var offset : float = clampf(
					pos.x,
					2.0,
					self.size.x - 5.0
				)
			entrances_down.append( offset )
	pass
...
func create_transition_blocks() -> void:
	...
	for c in transition_block.get_children():
		c.queue_free()
	...
	# DOWN
	for t in entrances_down:
		var block: ColorRect = add_block()
		block.size.x = 3
		block.position = Vector2(t, size.y - 1)
		
func add_block() -> ColorRect:
	var block : ColorRect = ColorRect.new()
	transition_block.add_child( block )
